  ...
  const handleIntegrateBatch = useCallback((batch: BatchContactItem[]) => {
    setContacts((prev) => {
      ...
      const formatted = batch.map((item) => ({
        ...item,
        id: `GCC${String(next++).padStart(4, '0')}`,
        tipoVinculo: item.tipoVinculo || (item.email.toLowerCase().startsWith('t_') ? 'Terceiro' : 'Próprio')
      } as Contact));
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col text-slate-900 font-sans">
      ...
      {toast && (
        <div role="status" aria-live="polite" className="fixed bottom-6 right-6 z-50">
          <div className={`${toast.bgColor || 'bg-slate-900'} text-white px-4 py-3 rounded-xl shadow-2xl flex items-center gap-3 text-xs font-semibold max-w-md`}>
            <span>{toast.message}</span>
            <button onClick={() => setToast(null)} aria-label="Fechar notificação">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
